...
public class Inventory{
	...
	public String getAllServicesText() throws NoSuchItemException
	{
		String result = "";
		if(services.size() == 0)
			throw new NoSuchItemException("There is no service!");
		for(int i = 0; i < services.size(); i++)
			result += services.get(i).getServiceInfoText();
		result += "\n";
		return result;
	}
	...
	private String getServiceName(Service s) // ı added a helper func for clear code
	{
		String serviceName = "";
		if(s instanceof Item)
			serviceName = ((Item)s).getItemName();
		else if(s instanceof FoodDelivery)
			serviceName = ((FoodDelivery)s).getFoodName();
		else if(s instanceof Streaming)
			serviceName = ((Streaming)s).getTitle();

		return serviceName;

	}

	public void ft_check(String name,int quantity)throws NoSuchItemException
	{
		for(Service currentService : services)
		{
			String serviceName = getServiceName(currentService);
			...
			if(serviceName.equals(name))
			{
				if(quantity != -1)
				{
					...
					{
						Item currenItem = (Item) currentService;
						if(currenItem.getItemQuantity() < quantity)
	...
	public void ft_add(String name, int q, String region, String city, String postCode)
	{
		for(Service currentService : services)
		{
			String serviceName = getServiceName(currentService);
	...
	public String get_text_fee_limit(int limit)
	{
		ListIterator<Service> it = services.listIterator();
		ArrayList<Service> temp_list = new ArrayList<Service>();
		String result = "";
		...
		if(temp_list.size() != 0)
		{
			for(Service loop : temp_list) {
				...
				if(loop instanceof Item) 
					...
					serviceName = ((Streaming)loop).getTitle();
				result += "Service:" + serviceName + " Shipping Fee: " + loop.calculateShippingFee() + "\n";
			}
			return result;			
		}
		result += "There is no service with a shipping fee over this limit.";
		return result;
	}
